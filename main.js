...
function drawPixel(pixelsArray) {
  ...
  for (i = 0; i < gridSize; i++) {
    ...
    for (j = 0; j < gridSize; j++) {
      ...
      ctx.fillStyle = color;
      ctx.fillRect(
        mouseZoom[0] + i * cellsSize,
        mouseZoom[1] + j * cellsSize,
        cellsSize,
        cellsSize
      );
    }
...
function checkIsAlive(cellState, neighboursNbrCellAlive) {
  ...
  if (!cellState && neighboursNbrCellAlive === 3) {
    ...
    cellState &&
    neighboursNbrCellAlive > 1 &&
    neighboursNbrCellAlive < 4
  ) {
    return true;
  } else {
    return false;
...
canvas.addEventListener("click", function (e) {
  ...
  drawGrid[coordinate[0]][coordinate[1]] = drawGrid[coordinate[0]][
    coordinate[1]
  ]
    ? false
    : true;
